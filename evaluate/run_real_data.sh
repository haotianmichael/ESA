...
READ_IDS="${READ_IDS:-}"            # optional file of read_ids to restrict SquiggleSeek to (e.g. held-out test split)
...
echo "trim_mode=$TRIM_MODE  trim_fixed=$TRIM_FIXED  read_ids=${READ_IDS:-<all>}"
...
[ -n "$READ_IDS" ] && SS_ARGS+=( --read_ids "$READ_IDS" )
if [ -n "$TRUTH_PAF" ]; then
